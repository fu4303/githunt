...
const loadPreferncePromise = preferredStorage
.getItem('HitUP:preference:GitHubTrending').then(data => {
...
function usePreference(initial = {}, options = {}) {
  ...
  **/
  ...
  if (!state._rehydrated) {
    loadPreferncePromise.then(pref => {
      ...
      <div className="header-row">
        <div className="group-heading">
          ...
          <span className="small text-muted ml-2">
            {trendingPeriodDefs[preference.since].heading}
          </span>
        </div>
